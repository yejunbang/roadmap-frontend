vue 生命周期：
beforeCreated：此时的vue实例还没有挂载元素$el，完成实例初始化， this指向创建的实例；可以在这加个loading事件；
data computed watch methods上的方法和数据均不能访问
created：实例创建完成，但是$el还没有，完成数据(data props computed)的初始化 导入依赖项。
...
子组件mounted
父组件mounted
父组件先创建，但是遇到子组件会先完成子组件的渲染和挂载，最后才挂载自己